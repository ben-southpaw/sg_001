...
interface SearchFilterProps {
  ...
  onFilterChange: (filterType: 'regions' | 'industries' | 'experienceLevels', values: string[]) => void;
}
...
const SearchFilter: React.FC<SearchFilterProps> = ({
        ...
        {items.length === 0 ? (
          ...
          items.sort().map(item => (
            <div key={item} className="flex items-center">
              ...
              />
              <label htmlFor={`${filterType}-${item}`} className="ml-2 block text-sm text-gray-700 truncate">
                {item}
              </label>
            </div>
          ))
  ...
  const hasFilters = selectedFilters.regions.length > 0 || 
